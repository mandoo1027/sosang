...
const callApi1 = () => {
  ...
  admApi
    .ADM0304S01(params)
    .then((response) => {
      const containerIdList = response.RES_DAT?.containerIdList
      if (response.RES_DAT?.containerIdList?.length > 0)
...
const callApi = () => {
  ...
  admApi
    .ADM0304S02(params)
    .then((response) => {
      if (response.RES_DAT?.searchList?.length > 0) {
        logInfos.value = response.RES_DAT.searchList
