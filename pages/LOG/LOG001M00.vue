  ...
  loginApi
    ...
    .then((response) => {
      ...
      if (response.RES_DAT.result) {
        if (response.RES_DAT.resultCode === 'CERT') {
          params.serialNo = response.RES_DAT.serialNo
          //추가인증
          $fx
            ...
            .then((result) => {
              if (result.isPass) {
                dupliLoginChk(result?.response?.RES_DAT?.resultCode)

                successLogin(result.response.RES_DAT.member)
              }
            })
        } else {
          dupliLoginChk(response?.RES_DAT?.resultCode)
          //로그인 처리
          successLogin(response.RES_DAT.member)
        }
      } else {
        loginData.isFail = true
        // if (response.RES_DAT.resultCode === 'R1') {
        //   //회원정보가 없거나 탈퇴한 경우
        //   loginData.isFail = true
        //   isPasswordFiveFail.value = false //비밀번호 5회 실패 초기화
        // } else if (response.RES_DAT.resultCode === 'R2') {
        //   //비밀번호 5회 실패시
